    ...
    return () => {
      ...
      if (isMultiType) {
        ...
        return items.map((s: Schema, index: number) => {
          ...
          const us = Array.isArray(uiSchema.items) ? uiSchema.items[index] || {} : uiSchema || {};
          return <SchemaItem schema={s} key={index} uiSchema={us} rootSchema={rootSchema} value={arr[index]} errorSchema={errorSchema[index] || {}} onChange={(v: any) => handleArrayItemChange(v, index)} />;
        });
      } else if (!isSelect) {
        const arr = Array.isArray(value) ? value : [];
        ...
        return arr.map((v: any, index: number) => {
          return (
            <ArrayItemWrapper index={index} onAdd={handleAdd} onDelete={handleDelete} onDown={handleDown} onUp={handleUp}>
              <SchemaItem schema={schema.items as Schema} uiSchema={(uiSchema.items as any) || {}} errorSchema={errorSchema[index] || {}} value={v} key={index} rootSchema={rootSchema} onChange={(v: any) => handleArrayItemChange(v, index)} />
            </ArrayItemWrapper>
          );
        });
      } else {
        ...
        return <SelectionWidget onChange={props.onChange} value={props.value} options={options} errors={errorSchema.__errors} schema={schema} />;
      }
    };
  },
});
