...
export function retrieveSchema(schema: any, rootSchema = {}, formData: any = {}): Schema {
  ...
  const hasAdditionalProperties = hasOwnProperty(resolvedSchema, 'additionalProperties') && resolvedSchema.additionalProperties !== false;
...
function resolveDependencies(schema: any, rootSchema: any, formData: any): Schema {
...
function withDependentSchema(schema: any, rootSchema: any, formData: any, dependencyKey: any, dependencyValue: any) {
  ...
  const resolvedOneOf = oneOf.map((subschema) => (hasOwnProperty(subschema, '$ref') ? resolveReference(subschema, rootSchema, formData) : subschema));
